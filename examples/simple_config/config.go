...
func newSubLoader() subLoader {
	...
	return subLoader{
		...
		IP:   ip,
		Port: port,
		Tls:  httpconf.TlsConfigLoader{TlsEnabled: optional.SomeBool(true), Certificate: cert, PrivateKey: key},
		//TlsCert: cert,
		//TlsKey:  key,
	}
}
...
func (l *subLoader) LoadFlags() {
	...
	l.Tls.Certificate = optional.Or(cert, l.Tls.Certificate)
	l.Tls.PrivateKey = optional.Or(key, l.Tls.PrivateKey)
